function Game() {
	...
	this.addPropertyToAuctionQueue = function(propertyIndex) {
		// Only add to auction queue when bidding is enabled in settings
		if (!this.biddingEnabled) return;
		auctionQueue.push(propertyIndex);
	};

	// Apply settings from the UI controls (exposed as a method on the game instance)
	this.applySettings = function() {
		// Check elements exist (page may not have loaded UI yet)
		var biddingEl = document.getElementById("biddingEnabled");
		var mortgageEl = document.getElementById("mortgageEnabled");
		var collectEl = document.getElementById("collectRentInJail");
		var startMoneyEl = document.getElementById("startingMoney");

		if (biddingEl) this.biddingEnabled = biddingEl.checked;
		if (mortgageEl) this.mortgageEnabled = mortgageEl.checked;
		if (collectEl) this.collectRentInJail = collectEl.checked;
		if (startMoneyEl) {
			var v = parseInt(startMoneyEl.value, 10);
			if (!isNaN(v)) this.startingMoney = Math.max(0, Math.min(3000, v));
		}

		// Update mortgage button visibility if manage panel is open
		if (typeof $ !== 'undefined') {
			if (!this.mortgageEnabled) {
				$("#mortgagebutton").hide();
			} else {
				$("#mortgagebutton").show();
			}
		...
		popup("<p>Settings applied.</p>");
	};

	this.auction = function() {
		// If bidding is disabled, skip auction
...
function Player(name, color) {
	...
	this.bidding = true;
	this.human = true;
	this.avatar = null; // Avatar filename (bubu.png, dudu.png, labubu.png, winnie.png, or null for color only)
	// this.AI = null;
...
function updatePosition() {
	...
	for (var x = 0; x < 40; x++) {
		...
		for (var y = turn; y <= pcount; y++) {
			...
			if (player[y].position == x && !player[y].jail) {
				...
				if (player[y].avatar) {
					var tokenSize = 50;
					var tokenOffset = Math.floor((tokenSize - 12) / 2);
					var leftPos = left - tokenOffset;
					var topPos = top - tokenOffset;
					if (leftPos < 0) leftPos = 0;
					if (topPos < 0) topPos = 0;
					tokenHTML = "<img src='avatar/" + player[y].avatar + "' class='player-token' title='" + player[y].name + "' style='position:absolute; left: " + leftPos + "px; top: " + topPos + "px; width:" + tokenSize + "px; height:" + tokenSize + "px;'/>";
				} else {
					tokenHTML = "<div class='cell-position' title='" + player[y].name + "' style='background-color: " + player[y].color + "; left: " + left + "px; top: " + top + "px;'></div>";
				}
				document.getElementById("cell" + x + "positionholder").innerHTML += tokenHTML;
				if (left == 36) {
					left = 0;
					top = 12;
				} else
	...
	for (var i = turn; i <= pcount; i++) {
			...
			if (player[i].avatar) {
				...
				if (leftPos < 0) leftPos = 0;
				if (topPos < 0) topPos = 0;
				tokenHTML = "<img src='avatar/" + player[i].avatar + "' class='player-token' title='" + player[i].name + "' style='position:absolute; left: " + leftPos + "px; top: " + topPos + "px; width:" + tokenSize + "px; height:" + tokenSize + "px;'/>";
			} else {
				tokenHTML = "<div class='cell-position' title='" + player[i].name + "' style='background-color: " + player[i].color + "; left: " + left + "px; top: " + top + "px;'></div>";
			}
			document.getElementById("jailpositionholder").innerHTML += tokenHTML;
...
function startBiddingProcess() {
	var p = player[turn];
	$("#landed").hide();
	addAlert(p.name + " declined to buy and started bidding for " + square[p.position].name + ".");
...
function declinePurchase() {
	var p = player[turn];
	$("#landed").hide();
	addAlert(p.name + " declined to purchase " + square[p.position].name + ". Turn ended.");
	play();
}

function mortgage(index) {
	if (typeof game !== 'undefined' && !game.mortgageEnabled) {
		return false;
	}
...
function land(increasedRent) {
	...
	if (s.price !== 0 && s.owner === 0) {
		...
		if (!p.human) {
			...
			// Show property purchase options based on settings
			var buyButtonHTML = "<div>You landed on <a href='javascript:void(0);' onmouseover='showdeed(" + p.position + ");' onmouseout='hidedeed();' class='statscellcolor'>" + s.name + "</a>.</div>";
			buyButtonHTML += "<div style='margin-top: 10px;'>";
			buyButtonHTML += "<input type='button' onclick='buy();' value='Direct Buy ($" + s.price + ")' title='Buy " + s.name + " directly for " + s.pricetext + ".'/>";
			// Only show bidding button if bidding is enabled
			if (game.biddingEnabled) {
				buyButtonHTML += "<input type='button' onclick='game.addPropertyToAuctionQueue(" + p.position + "); startBiddingProcess();' value='Start Bidding' title='Start an auction for this property.'/>";
			}
			buyButtonHTML += "<input type='button' onclick='declinePurchase();' value='Decline Purchase' title='Skip buying and let others bid.'/>";
			buyButtonHTML += "</div>";
			document.getElementById("landed").innerHTML = buyButtonHTML;
		}
	...
	// Collect rent
	var shouldCollectRent = s.owner !== 0 && s.owner != turn && !s.mortgage;
	
	// If property owner is in jail and collectRentInJail is disabled, don't collect rent
	if (shouldCollectRent && player[s.owner] && player[s.owner].jail && !game.collectRentInJail) {
		shouldCollectRent = false;
	}
...
function roll() {
	...
	// Helper to animate player movement step-by-step
	function movePlayerWithSteps(p, steps, delay, callback) {
		var remaining = steps;
		var stepFn = function() {
			if (remaining <= 0) {
	...
	if (p.jail === true) {
		...
		document.getElementById("nextbutton").disabled = true;
		var steps = die1 + die2;
		movePlayerWithSteps(p, steps, 300, function() {
...
// Global wrapper so the HTML 'Apply Settings' button can call this
function applySettings() {
	if (window.game && typeof game.applySettings === 'function') {
		game.applySettings();
	}
}
...
function setup() {
	...
	if (typeof game !== 'undefined' && typeof game.applySettings === 'undefined') {
		// nothing
	}
	...
	for (var i = 1; i <= pcount; i++) {
		...
		// Read avatar selection from UI
		var avatarEl = document.getElementById("player" + i + "avatar");
		p.avatar = avatarEl ? avatarEl.value : "none";
		if (p.avatar === "none") p.avatar = null;
		
		// Set starting money from settings
		if (game && typeof game.startingMoney !== 'undefined') {
			p.money = game.startingMoney;
		} else {
			p.money = 1500;
		}

		// Apply bidding preference per player
		if (game && typeof game.biddingEnabled !== 'undefined') {
		...
		if (moneyNameEl) {
			moneyNameEl.innerHTML = p.name + (p.avatar ? " (" + p.avatar + ")" : "");
		}
	}
